...
class Slime(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx == 0:  # Only print for first batch
            print(f"\n=== Mask Debug Info ===")
            print(f"Number of parts (self.num_parts): {self.num_parts}")
            print(f"Part names: {self.config.part_names}")
            print(f"Unique values in mask: {values.cpu().numpy()}")
            print(f"Mask min value: {mask.min().item()}")
            print(f"Mask max value: {mask.max().item()}")
            print(f"Expected mask range: [0, {self.num_parts - 1}]")
            
            # Check for out-of-bounds values
            out_of_bounds = values[values >= self.num_parts]
            if len(out_of_bounds) > 0:
                print(f"WARNING: Found out-of-bounds mask values: {out_of_bounds.cpu().numpy()}")
                print(f"These values will be clamped to {self.num_parts - 1}")
            
            print(f"======================\n")
        
        # Filter out values that are out of bounds
        valid_mask = values < self.num_parts
        valid_values = values[valid_mask]
        valid_counts = counts[valid_mask]
        
        # Only assign counts for valid indices
        if len(valid_values) > 0:
            num_pixels[valid_values.type(torch.int64)] = valid_counts.type(torch.int64)
        ...
        loss1 = F.cross_entropy(
            ...
            torch.clamp(mask[None, ...], 0, self.num_parts - 1).type(torch.long),
        ...
        # Clip mask values to valid range before using scatter_
        clipped_mask = torch.clamp(mask, 0, self.num_parts - 1)
        one_shot_mask = (
            torch.zeros(
                self.num_parts,
                mask.shape[0],
            ...
            .to(mask.device)
            .scatter_(0, clipped_mask.unsqueeze(0).type(torch.int64), 1.0)
        )
