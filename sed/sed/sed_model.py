...
class SED(nn.Module):
    ...
    def instances_to_semantic_seg(self, instances, height, width, num_classes):
        """
        Convert instance annotations to semantic segmentation format.
        ...
        Returns:
            sem_seg: semantic segmentation mask tensor
        """
        sem_seg = torch.zeros((height, width), dtype=torch.long, device=self.device)
        ...
        if hasattr(instances, 'gt_masks'):
            ...
            if hasattr(gt_masks, 'tensor'):
                ...
                # For polygon masks, convert to bitmasks
                masks = BitMasks.from_polygon_masks(gt_masks, height, width).tensor
            
            # Assign pixels to classes (later instances override earlier ones)
            for mask, cls in zip(masks, gt_classes):
                sem_seg[mask > 0] = cls
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            # Handle both sem_seg and instances formats
            targets = []
            for x in batched_inputs:
                if "sem_seg" in x:
                    targets.append(x["sem_seg"].to(self.device))
                elif "instances" in x:
                    # Convert instances to semantic segmentation
                    height = x.get("height", x["image"].shape[-2])
                    width = x.get("width", x["image"].shape[-1])
                    num_classes = outputs[0].shape[1]
                    sem_seg = self.instances_to_semantic_seg(x["instances"], height, width, num_classes)
                    targets.append(sem_seg)
                else:
                    raise ValueError("batched_inputs must contain either 'sem_seg' or 'instances' key")
