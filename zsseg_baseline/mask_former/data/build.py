...
def dataset_sample_per_class(cfg):
    ...
    # Check if this is a semantic segmentation dataset (no category_id field)
    if len(dataset_dicts) > 0 and "category_id" not in dataset_dicts[0]:
        logger = logging.getLogger(__name__)
        logger.info("Semantic segmentation dataset detected - skipping per-class sampling")
        return dataset_dicts
    ...
    # Only print classification histogram if category_id exists
    if len(dataset_dicts) > 0 and "category_id" in dataset_dicts[0]:
