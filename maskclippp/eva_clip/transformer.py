...
class Attention(nn.Module):
    ...
    def forward(self, x, attn_mask: Optional[torch.Tensor] = None):
        ...
        if self.xattn:
            q = q.contiguous().view(L, N, self.num_heads, -1).permute(1, 2, 0, 3).contiguous()
            k = k.contiguous().view(L, N, self.num_heads, -1).permute(1, 2, 0, 3).contiguous()
            v = v.contiguous().view(L, N, self.num_heads, -1).permute(1, 2, 0, 3).contiguous()
            ...
            x = torch.bmm(attn, v)
            x = x.view(N, self.num_heads, L, -1)
        if self.head_scale is not None:
            x = x * self.head_scale
        x = x.permute(2, 0, 1, 3).contiguous().view(L, N, C)
        x = self.out_proj(x)
        x = self.out_drop(x)
        return x
...
class CustomAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
        ...
        N_v, B_v, C_v = v.shape
        if self.xattn:
            # N, B, C -> B, N, C -> B, N, num_heads, C -> B, nh, N, nc
            q = q.contiguous().view(N_q, B_q, self.num_heads, -1).permute(1, 2, 0, 3).contiguous()
            k = k.contiguous().view(N_k, B_k, self.num_heads, -1).permute(1, 2, 0, 3).contiguous()
            v = v.contiguous().view(N_v, B_v, self.num_heads, -1).permute(1, 2, 0, 3).contiguous()
            ...
            attn = self.attn_drop(attn)
            x = torch.bmm(attn, v)
            x = x.view(B_q, self.num_heads, N_q, -1)
        if self.head_scale is not None:
            x = x * self.head_scale
        x = x.permute(2, 0, 1, 3).contiguous().view(N_q, B_q, C_q)
        x = self.out_proj(x)
        x = self.out_drop(x)
        return x
