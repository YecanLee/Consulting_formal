...
class Attention(nn.Module):
    def __init__(
        ...
        if self.xattn:
            assert self.relative_position_bias_table is None
    ...
    def forward(self, x, rel_pos_bias=None, attn_mask=None):
        ...
        if self.rope:
            # slightly fast impl
            hw = self.rope.ft_seq_size[0] * self.rope.ft_seq_size[1]
            start_i = N - hw
            q_t = q[:, :, start_i:, :]
            ro_q_t = self.rope(q_t)
            q = torch.cat((q[:, :, :start_i, :], ro_q_t), -2).type_as(v)

            k_t = k[:, :, start_i:, :]
            ro_k_t = self.rope(k_t)
            k = torch.cat((k[:, :, :start_i, :], ro_k_t), -2).type_as(v)
