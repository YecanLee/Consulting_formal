...
def register_ceiling_dataset():
    # Register training data with custom loader
    DatasetCatalog.register("ceiling_easy_train", lambda: load_ceiling_data_with_sem_seg(TRAIN_JSON, TRAIN_DIR, "ceiling_easy_train"))
    MetadataCatalog.get("ceiling_easy_train").set(
        evaluator_type="sem_seg",
        stuff_classes=["background", "brief", "mural", "relief"],  
        ignore_label=255,
    )
    print("Registered ceiling_easy_train with sem_seg")
    
    # Use the default register_coco_instances for valid data
    DatasetCatalog.register("ceiling_easy_val", lambda: load_ceiling_data_with_sem_seg(VAL_JSON, VAL_DIR, "ceiling_easy_val"))
    MetadataCatalog.get("ceiling_easy_val").set(
        evaluator_type="sem_seg",
        stuff_classes=["background", "brief", "mural", "relief"],  
        ignore_label=255,
    )
    print("Registered ceiling_easy_val with sem_seg")
